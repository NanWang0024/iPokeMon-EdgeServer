    ...
    def check_name_uniqueness(self, p_username):
        ...
        if self.redis.sismember("usernames", p_username):
            ...
            # otherwise, name is not unique
                ...
                return False
        # not exists, this name is unique, try to set new for user
...
def index():
    ...
    return 'This is Edge Server Running.'
...
def get_timestamp():
	print 'startTime:', time.time() * 1000
@server.hook('after_request')
def get_timestamp():
	print 'endTime:', time.time() * 1000
...
run(
        ...
        host    =   '0.0.0.0',    # Set 0 to use LXC port forwarding
    port = 8000,               
